-- ==========================================
-- ANIMAL HOSPITAL – XDHub (All-in-One)
-- Loads Rayfield, backend, and GUI.
-- Fully self-contained – no external files.
-- ==========================================
...
-- Expose globally (so the GUI can modify it)
        ...
        if Config.AutoBarneyCoffee then
            pcall(function()
                ...
                if npcs then
                    ...
                    if barney then
                        local args = { [1] = "GiveCoffee", [2] = barney }
...
-- ==========================================
-- AUTO TREAT PATIENTS (via Remote)
-- ==========================================
task.spawn(function()
    while true do
        task.wait(0.5)
                ...
                if patients then
                    for _, v in ipairs(patients:GetChildren()) do
                        if v:FindFirstChild("HumanoidRootPart") then
                            GetRootPart().CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0, 0, 2)
                            local remotes = ReplicatedStorage:FindFirstChild("Remotes")
                            if remotes then
...
-- This is the GUI part (originally main.lua)
-- It uses the Utility and Config from above.
